...
exports.main = async(event, context) => {
  ...
  for (let i = 0; i < batchTimes; i++) {
    const promise = db.collection('talklist').where(
      _.or([
        {
          masterid:openid
        },
  ...
  const talkListByOpenIdList = (await Promise.all(tasks)).reduce((acc, cur) => {
    return {
  ...
  for (let i = 0; i < talkListByOpenIdList.data.length; i++) {
    const promise2 = await db.collection('eatmeet').orderBy('datetime', 'desc').where({
